            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-left mb-0">Mevduatlar</h2>
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                                    <li class="breadcrumb-item"><a href="{{ route('admin.deposits.index') }}">Mevduatlar</a></li>
                                </ol>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card">
                                <table class="datatables-basic table">
                                    ...
                                    <tr>
                                        ...
                                        <th>BAKİYE</th>
                                        <th>PARA BİRİMİ</th>
                                        <th>GELİR KOMİSYON</th>
                                        <th>GİDER KOMİSYON</th>
                                        <th>İŞLEMLER</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($deposit as $key => $row)
                                        <tr>
                                            ...
                                            <td>{{ $row->title }}</td>
                                            <td>
                                                @php($query = \App\Models\Group::where('id',$row->group_id)->first())
                                                @if($row->group_id != NULL && isset($query) && $query)
                                                    <div class="badge badge-glow badge-{{ \App\Models\Group::find($row->group_id)->color  }}">
                                                        {{ \App\Models\Group::find($row->group_id)->title  }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td>{{ $row->balance }}</td>
                                            <td>{{ $row->currency }}</td>
                                            <td>%{{ $row->income_commission }}</td>
                                            <td>%{{ $row->expense_commission }}</td>
                                            {{--<td>{{ $row->date }}</td>--}}
